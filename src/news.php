...
<head>
    ...
    <title>Berita - TK Pertiwi 14</title>
...
<body>
    ...
    <section class="py-5 bg-success text-white">
        <div class="container text-center">
            <h1 class="display-4 fw-bold mb-3"><i class="fas fa-newspaper"></i> Berita Terbaru</h1>
            <p class="lead">Informasi dan Kegiatan TK Pertiwi 14</p>
        </div>
    </section>
        ...
        <div class="container">
            <div class="row g-4">
                ...
                if ($result->num_rows > 0) {
                    // Display all articles in a uniform grid
                    while($row = $result->fetch_assoc()) {
                        ...
                        echo "<img src='" . $image . "' class='card-img-top' alt='" . htmlspecialchars($row['title']) . "' style='height: 200px; object-fit: cover;' onerror=\"this.src='" . get_placeholder_image_svg(400, 300) . "'\">";
                        echo "<div class='card-body'>";
                        echo "<h5 class='card-title'>" . htmlspecialchars($row["title"]) . "</h5>";
                        echo "<p class='card-text text-muted'>" . htmlspecialchars(substr($row["content"], 0, 120)) . "...</p>";
                        echo "<div class='d-flex justify-content-between align-items-center'>";
                        echo "<small class='text-muted'><i class='far fa-calendar'></i> " . date('d M Y', strtotime($row["created_at"])) . "</small>";
                        echo "<a href='article.php?id=" . $row['id'] . "' class='btn btn-sm btn-outline-success'>Baca <i class='fas fa-arrow-right'></i></a>";
                        echo "</div>";
                        echo "</div>";
                        echo "</div>";
                        echo "</div>";
                    ...
                    echo "<i class='fas fa-info-circle'></i> Belum ada berita yang dipublikasikan.";
                    echo "</div>";
                    echo "</div>";
                }
                $conn->close();
